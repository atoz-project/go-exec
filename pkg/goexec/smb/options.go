...
type ClientOptions struct {
	...
	// ConnectTimeout controls how long TCP connect is allowed to take.
	// A zero value means no explicit timeout (caller context may still impose one).
...
func (c *Client) Parse(ctx context.Context) (err error) {
	...
	if c.Proxy == "" {
		c.netDialer = &net.Dialer{Timeout: c.ConnectTimeout}
	...
	if c.Port == 0 {
		if c.Target != nil && c.Target.Port != "" {
			p, parseErr := strconv.ParseUint(c.Target.Port, 10, 16)
			if parseErr != nil {
				return fmt.Errorf("parse %s target port %q: %w", ClientName, c.Target.Port, parseErr)
			}
			c.Port = uint16(p)
		} else {
			c.Port = DefaultPort
