...
func (o *OutputFileFetcher) GetOutput(ctx context.Context, writer io.Writer) (err error) {
	...
	if reader, err := func() (io.ReadWriteCloser, error) {
		timer := time.NewTimer(timeout)
		defer timer.Stop()
		poll := time.NewTicker(pollInterval)
		defer poll.Stop()

		// Try once immediately before waiting for the ticker.
		if r, openErr := shareOpenFile(o.Client.mount, o.relativePath, os.O_RDWR, 0); openErr == nil {
			return r, nil
		}
		...
		for {
			...
			case <-poll.C:
				...
				r, openErr := shareOpenFile(o.Client.mount, o.relativePath, os.O_RDWR, 0)
				if openErr == nil {
