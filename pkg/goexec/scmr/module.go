...
func (m *Scmr) deleteService(ctx context.Context, svc *service) (err error) {
	...
	if deleteResponse.Return != 0 {
		codeErr := win32.FromCode(deleteResponse.Return)
		log.Error().Err(codeErr).Str("code", fmt.Sprintf("0x%08x", deleteResponse.Return)).Msg("Failed to delete service")
		return fmt.Errorf("delete service: %w", codeErr)
	}
...
func (m *Scmr) closeService(ctx context.Context, svc *service) (err error) {
	...
	if closResponse.Return != 0 {
		codeErr := win32.FromCode(closResponse.Return)
		log.Error().Err(codeErr).Str("code", fmt.Sprintf("0x%08x", closResponse.Return)).Msg("Failed to close service handle")
		return fmt.Errorf("close service: %w", codeErr)
	}
