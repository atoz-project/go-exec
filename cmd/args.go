...
// This would upload a local file to the remote host via SMB, then execute it.
// Requires SMB write support which is not yet implemented.
/*
...
func argsTarget(proto string) func(cmd *cobra.Command, args []string) error {
	...
	return func(cmd *cobra.Command, args []string) (err error) {
		...
		if credential, target, err = adAuthOpts.WithTarget(context.Background(), proto, args[0]); err != nil {
...
func argsSmbClient() func(cmd *cobra.Command, args []string) error {
	return args(
		...
		func(_ *cobra.Command, _ []string) error {
			...
			return smbClient.Parse(context.Background())
		},
	)
}
...
func argsRpcClient(proto string, endpoint string) func(cmd *cobra.Command, args []string) error {
	return args(
		...
		func(cmd *cobra.Command, args []string) (err error) {
			...
			return rpcClient.Parse(context.Background())
		},
	)
}
